...
const GoogleCalendarModal = ({ events, isLoading, onClose, onAddGoal }) => {
    ...
    let content;
    if (isLoading) {
        content = React.createElement('div', { className: "flex justify-center items-center h-full" }, React.createElement(Spinner, null));
    } else if (events.length === 0) {
        content = React.createElement('div', { className: "flex justify-center items-center h-full text-slate-500" }, React.createElement('p', null, "No events found for today."));
    } else {
        content = React.createElement('ul', { className: "space-y-3" },
            ...events.map(event =>
                React.createElement('li', { key: event.id, className: "p-3 bg-slate-900/50 border border-slate-700 rounded-lg text-left flex items-center justify-between gap-4" },
                    React.createElement('div', null,
                        React.createElement('p', { className: "font-mono text-sm text-cyan-300" }, formatEventTime(event)),
                        React.createElement('p', { className: "font-bold text-white mt-1" }, event.summary)
                    ),
    ...
    return React.createElement('div', {
        ...
        React.createElement('div', {
            ...
            React.createElement('h2', { className: "text-2xl font-semibold mb-4 text-cyan-300" }, "Today's Calendar Events"),
